...
新手首束花从 v0.4 起按 Gemini 生成的手绘概念图路线重设，视觉目标参考本地文件：
...
绿叶、包装纸、蓝白条纹丝带和小吊牌由首个顾客订单自动提供。首版 DIY 不让玩家自由设计包装，而是用模板式插花保证第一束花稳定漂亮。
...
当前已进入首束花视觉垂直切片 v0.4：运行时花材统一切到插花枝材资产，DIY 与交付页复用同一套模板式花束状态。整体 UI 仍是原型级，花束模板和包装还需要继续审美调校；本地存档仍未实现。
