...
- 每个花头放置成功后切换为与 `template-base` 同尺寸的透明叠层，避免运行时小图锚点、裁切和缩放导致错位。
...
当前运行时已临时收束到首束花制作验证 `cutout-v02`：只显示花束制作页，使用 `template-base + 4 个拖拽花头 + 4 个同尺寸透明完成叠层` 验证玩家拖入花头后能否形成更自然的花束。完整种植/订单闭环代码仍保留在工程中，但本轮不继续扩展，等花束审美验证通过后再重新接回主循环。本地存档仍未实现。
